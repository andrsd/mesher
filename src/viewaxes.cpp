               ...
               double value_p2[3])
{
    ...
    double l = norme(t);
    double value_t[3] = { value_p2[0] - value_p1[0],
                          value_p2[1] - value_p1[1],
    ...
    drawString(label, p2[0] + t[0] * w2, p2[1] + t[1] * w2, p2[2] + t[2] * w2);
    ...
    world2Viewport(p1, win1);
    world2Viewport(p2, win2);
...
static void
drawGridStipple(int n1, int n2, double p1[3], double p2[3], double p3[3])
{
               ...
               int mikado)
{
    if (mikado) {
        ...
        if (ntics < 1)
            ntics = 1;
        double dd[3] = { (xmax - xmin) / ntics, (ymax - ymin) / ntics, (zmax - zmin) / ntics };
        double axe_color[4];
               ...
               double value_bb[6])
{
    ...
    if (mode == 3) {
        ...
        if (zmin != zmax)
            drawGridStipple(nx, ny, orig2, yz, xz);
        if (xmin != xmax)
            drawGridStipple(ny, nz, orig2, xz, xy);
        if (ymin != ymax)
            drawGridStipple(nx, nz, orig2, yz, xy);
    }
}
               ...
               SBoundingBox3d & value_bb)
{
    ...
    double value_bbox[6] = { value_bb.min().x(), value_bb.max().x(), value_bb.min().y(),
                             value_bb.max().y(), value_bb.min().z(), value_bb.max().z() };
...
View::drawAxes()
{
    ...
    if (geometryExists && (ctx->drawBBox || !ctx->mesh.draw)) {
        glColor4ubv((GLubyte *) &ctx->color.fg);
        glLineWidth((float) ctx->lineWidth);
        // gl2psLineWidth(
        //     (float) (ctx->lineWidth * ctx->print.epsLineWidthFactor));
        drawBox(ctx->min[0], ctx->min[1], ctx->min[2], ctx->max[0], ctx->max[1], ctx->max[2]);
        glColor3d(1., 0., 0.);
        for (int j = 0; j < 6; j++)
            if (ctx->geom.clip & (1 << j) || ctx->mesh.clip & (1 << j))
                drawPlaneInBoundingBox(ctx->min[0],
                                       ctx->min[1],
                                       ctx->min[2],
                                       ctx->max[0],
                                       ctx->max[1],
    ...
    if (ctx->axes) {
        ...
        if (!ctx->axesAutoPosition) {
            drawAxes(ctx->axes,
                     ...
                     ctx->axesPosition,
                     ctx->axesMikado,
                     ctx->axesForceValue ? ctx->axesValue : ctx->axesPosition);
        }
        else {
            ...
            drawAxes(ctx->axes,
                     ...
                     bb,
                     ctx->axesMikado,
                     ctx->axesForceValue ? ctx->axesValue : bb);
        }
    }
    ...
    if (this->draw_rotation_center) {
        ...
        if (ctx->rotationCenterCg)
            ...
            drawSphere(ctx->pointSize,
                       ctx->rotationCenter[0],
                       ctx->rotationCenter[1],
                       ctx->rotationCenter[2],
                       ctx->geom.light);
    }
}
